...
export default function ProgressTracker() {
  ...
  const getStats = () => {
    return {
      totalTime: hobbies.reduce((acc, hobby) => acc + (hobby.timeSpent || 0), 0),
      totalMilestones: hobbies.reduce((acc, hobby) => acc + (hobby.milestones?.filter((m) => m.completed)?.length || 0), 0),
      activeStreak: Math.max(...hobbies.map((h) => h.streak || 0), 0),
  ...
  const getProgressPercentage = (hobby) => {
    ...
    const completedMilestones = hobby.milestones.filter((m) => m.completed)?.length || 0
    return Math.round((completedMilestones / hobby.milestones.length) * 100)
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 px-4 sm:px-6 sm:space-y-8">
      {/* Stats Overview */}
        ...
        <div className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              ...
              <p className="text-base sm:text-xl font-semibold">{stats.totalTime}h</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-xs sm:text-sm text-gray-500">Milestones</p>
              <p className="text-base sm:text-xl font-semibold">{stats.totalMilestones}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="bg-orange-100 p-1.5 sm:p-2 rounded-lg">
              <Fire className="h-4 w-4 sm:h-5 sm:w-5 text-orange-600" />
            </div>
            <div>
              <p className="text-xs sm:text-sm text-gray-500">Best Streak</p>
              <p className="text-base sm:text-xl font-semibold">{stats.activeStreak}d</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              ...
              <p className="text-base sm:text-xl font-semibold">{stats.totalHobbies}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
        {hobbies.map((hobby, index) => (
          <div
            key={index}
            className="bg-white rounded-xl shadow-md sm:shadow-xl p-4 sm:p-6 transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] border border-gray-100"
          >
            <div className="flex justify-between items-start mb-3 sm:mb-4">
              <div className="space-y-0.5 sm:space-y-1">
                <h2 className="text-xl sm:text-2xl font-semibold text-[#8B1E3F]">{hobby.name}</h2>
            ...
            <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
              <div className="flex justify-between items-center">
                <h3 className="font-medium text-[#8B1E3F] flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base">
                  <Trophy className="h-4 w-4 sm:h-5 sm:w-5" />
                  Milestones
                </h3>
                <button
                  onClick={() => setEditingMilestone(index)}
                  className="text-xs sm:text-sm bg-[#8B1E3F]/10 text-[#8B1E3F] px-2 sm:px-3 py-1 rounded-full hover:bg-[#8B1E3F]/20 transition-colors"
                >
                  Add Milestone
                </button>
              </div>
              ...
              {editingMilestone === index && (
                <div className="flex gap-2 animate-fade-in">
                  <input
                    ...
                    className="flex-1 border rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 text-sm focus:ring-2 focus:ring-[#8B1E3F]/20 focus:border-[#8B1E3F] outline-none"
                    placeholder="New milestone..."
                  />
                  <button
                    onClick={() => addMilestone(index)}
                    className="bg-[#8B1E3F] text-white rounded-lg px-2 sm:px-3 hover:bg-[#8B1E3F]/90 transition-colors"
                  >
                    <Save className="h-4 w-4 sm:h-5 sm:w-5" />
                  </button>
                  <button
                    onClick={() => setEditingMilestone(null)}
                    className="bg-gray-100 text-gray-600 rounded-lg px-2 sm:px-3 hover:bg-gray-200 transition-colors"
                  >
              ...
              <div className="space-y-1.5 sm:space-y-2">
                {hobby.milestones.map((milestone, idx) => (
                  <div key={idx} className="flex items-start gap-2 sm:gap-3 p-1.5 sm:p-2 hover:bg-gray-50 rounded-lg transition-colors">
            ...
            <div className="flex justify-between items-center text-xs sm:text-sm text-gray-600 bg-gray-50 p-2 sm:p-3 rounded-lg">
              <div className="flex items-center gap-2 sm:gap-3">
                <Clock className="h-3 w-3 sm:h-4 sm:w-4 text-[#8B1E3F]" />
                <span className="font-medium">{hobby.timeSpent}h</span>
                <div className="flex gap-1">
                  <button
                    onClick={() => updateTimeSpent(index, -1)}
                    className="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center bg-white rounded-md shadow-sm hover:bg-gray-100 transition-colors"
                  >
                    -
                  </button>
                  <button
              ...
              <select
                ...
                className="border rounded-lg px-2 sm:px-3 py-1 text-xs sm:text-sm bg-white focus:ring-2 focus:ring-[#8B1E3F]/20 focus:border-[#8B1E3F] outline-none"
              >
                {levels.map((level) => (
      ...
      <div className="mt-6 sm:mt-8 text-center">
        <button
          ...
          className="bg-gradient-to-r from-[#8B1E3F] to-[#E3425F] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:shadow-lg transition-all flex items-center gap-2 mx-auto text-sm sm:text-base"
        >
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-xl p-4 sm:p-6 w-full max-w-md shadow-2xl animate-scale-in">
